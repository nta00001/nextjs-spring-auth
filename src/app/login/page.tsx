...
export default function LoginPage() {
  ...
  const handleLogin = async (event: FormEvent) => {
    ...
    try {
      ...
      await getMyInfoAndRedirect(data.access_token);
  ...
  const getMyInfoAndRedirect = async (token: string) => {
     try {
       ...
       const roles = meData.user.roles.map((role: any) => role.code);
        ...
        if (roles.includes("admin")) {
          router.push('/dashboard'); // Điều hướng đến trang admin
        } else if (roles.includes("user")) {
          router.push('/'); // Điều hướng đến trang home cho user
        } else {
          // Xử lý trường hợp không có quyền phù hợp
  ...
  <Link href="/register" className="font-semibold text-indigo-600 hover:underline">
    Đăng ký ngay
        ...
        {/* Bạn có thể giữ lại phần này để debug hoặc xóa đi */}
        {userInfo && (
